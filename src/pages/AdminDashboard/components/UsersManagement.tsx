...
const UsersManagement = () => {
  ...
  useEffect(() => {
    const loadUsers = async () => {
      try {
        ...
        const usersList = storedUsers ? Object.values(JSON.parse(storedUsers)) : [];
        setUsers(usersList);
      } catch (error) {
  ...
  const handleCreateUser = async (userData: any) => {
    try {
      ...
      const usersObj = updatedUsers.reduce((acc, user) => ({ ...acc, [user.id]: user }), {});
  ...
  const handleUpdateUser = async (userId: string, updatedData: any) => {
    try {
      ...
      localStorage.setItem('users', JSON.stringify(
        updatedUsers.reduce((acc, user) => ({ ...acc, [user.id]: user }), {})
      ));
    } catch (error) {
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredUsers.map((user) => (
              <tr key={user.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <TableStatus status={user.status || 'active'} />
                </td>
