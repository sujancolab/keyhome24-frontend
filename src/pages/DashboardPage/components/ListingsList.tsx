...
import { Edit3, Eye, Trash2, Home, AlertCircle, MapPin, ChevronRight } from 'lucide-react';
...
const ListingsList: React.FC<ListingsListProps> = ({ listings, onDelete, onEdit }) => {
  const [editingListing, setEditingListing] = useState<UserListing | null>(null);
  const [viewingListing, setViewingListing] = useState<UserListing | null>(null);

  const handleEdit = (listing: UserListing) => {
    if (listing.hasBeenEdited) {
      alert('Cette annonce a déjà été modifiée une fois. Vous ne pouvez plus la modifier.');
      return;
    }
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap items-center gap-4">
                  <span className="font-medium text-red-600">{listing.price}</span>
                  <span className="text-sm text-gray-600">
                    {listing.views} vues
                  </span>
              ...
              <div className="flex sm:flex-col gap-2 w-full sm:w-auto">
                <button 
                  ...
                  title={listing.hasBeenEdited ? 'Cette annonce a déjà été modifiée' : 'Modifier l\'annonce'}
                >
